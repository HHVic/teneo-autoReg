...
class TeneoAutoref:
    ...
    def make_request(self, method, url, **kwargs):
        try:
            ...
            response = requests.request(method, url, **kwargs, impersonate='chrome116')
    ...
    def make_request_redirect(self, method, url, **kwargs):
        try:
            if self.proxies:
                kwargs['proxies'] = self.proxies
                kwargs['timeout'] = 60

            response = requests.request(method, url, **kwargs, allow_redirects=False, impersonate='chrome116')
            response.raise_for_status()
            return response
        except requests.exceptions.RequestException as e:
            log_message(self.current_num, self.total, f"Request failed: {str(e)}", "error")
    ...
    def verify_email(self, verification_url):
        ...
        real_url = self.fetch_location_header(verification_url)
        if not real_url:
            return False
        response = self.make_request('GET', real_url, headers={'User-Agent': self.ua.random}, timeout=60)
    ...
    def fetch_location_header(self, url):
        response = self.make_request_redirect('GET', url, headers={'User-Agent': self.ua.random}, timeout=60)
        ...
        if location:
            ...
            log_message(self.current_num, self.total, 'Location header not found.', "error")
            return None
